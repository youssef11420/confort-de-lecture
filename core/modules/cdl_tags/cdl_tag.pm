...
sub getCdlTagInnerHtmlAndInfos #($htmlCode, $cdlTag, @orderedSortAttributes)
{
	...
	$htmlCode =~ s/<!--$cdlTag(\s[^>]*?)?-->\s*(.+?)\s*(<!--\/$cdlTag-->)/
		my $cdlTagInfosHash;
...
sub parseAllNavsBlocs #($htmlCode, $siteRootUrl, $pagePath, $activateJavascript, $parseJavascript, $displayImages, $displayObjects, $displayApplets, $parseTablesToList, $activateFrames, $siteId, $pageUri, $trustedDomainNames, $entirePageTemplateString, $cadreTemplateString, $cdlTag, $templateMarker, %sortAttributes)
{
	...
	($htmlCode, @cdlTagsInfos) = getCdlTagInnerHtmlAndInfos($htmlCode, $cdlTag, keys(%sortAttributes));

	if (@cdlTagsInfos > 0) {
		my $content = $cdlTagsInfos[0]->{'content'};
			...
			foreach my $attributeKey (keys(%sortAttributes)) {
				if ($sortAttributes{$attributeKey}) {
					if ($cdlTagsInfos[$j-1]->{'attributes'}->{$attributeKey} eq $cdlTagsInfos[$j]->{'attributes'}->{$attributeKey}) {
						$content .= $cdlTagsInfos[$j]->{'content'}."\n";
