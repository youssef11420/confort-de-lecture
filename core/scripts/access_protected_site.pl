...
if ((param('cdlact') eq "c") and (param('cdlloginerror') ne "1")) {
	...
	if ($activateAudio eq "1") {
		...
		$protectedPageTemplateString = setValueInTemplateString($protectedPageTemplateString, 'AUDIO_SERVER_NAME', ($ttsMode eq "sdk" and $embeddedMode ne "") ? "recette.cdl.lnet.fr" : $ENV{'SERVER_NAME'}.$embeddedMode);
	} else {
