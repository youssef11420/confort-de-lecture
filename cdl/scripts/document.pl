...
if ($action) {
	...
	redirectDownload($action, $requestMethod, $urlToParse, $session, $siteId, %requestParameters);
} else {
	...
	$documentPageTemplateString = setValueInTemplateString($documentPageTemplateString, 'DOCUMENT_TYPE', $contentType);
