...
my ($action, $siteId, $contentType, $requestMethod, $defaultLanguage, $urlToParse);
...
if ((param('cdlact') eq "c") and (param('cdlloginerror') ne "1")) {
	...
	connectProtectedSite($cgi, $requestMethod, $urlToParse, $userLogin, $passwd, $realm, $session, $siteId, %requestParameters);
} else {
	...
	foreach my $param (keys(%requestParameters)) {
		if ($param !~ /^cdl/si) {
			$hiddenParams .= "\t\t\t\t\t\t\t\t<input type=\"hidden\" name=\"".$param."\" value=\"".$requestParameters{$param}."\" />\n";
		}
	}
