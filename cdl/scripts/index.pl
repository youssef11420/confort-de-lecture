...
$thisCdlUrl =~ s/^\/le\-filtre\-https\/(.*?)(\/(([^\?]*?)(\?.*)?)?)?$/$siteId = urlDecode($1); $pageUri = $httpMethod eq "POST" ? $3 : urlDecode($4); $secure = "s";/segi;
$thisCdlUrl =~ s/^\/le\-filtre\/(.*?)(\/(([^\?]*?)(\?.*)?)?)?$/$siteId = urlDecode($1); $pageUri = $httpMethod eq "POST" ? $3 : urlDecode($4);/segi;
