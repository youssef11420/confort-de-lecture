...
sub cleanIllegalChars #($htmlCode)
{
	...
	$htmlCode =~ s/&();/_cdl_carac_temp_$1_/sgi;
	$regExp = "&(";
...
sub initParametersForRequest #($request, $methodKey, %requestParameters)
{
	...
	my $requestParametersString = "";
	foreach my $paramKey (keys(%requestParameters)) {
		$requestParametersString .= $paramKey."=".urlEncode($requestParameters{$paramKey})."&";
	}
...
sub getReferer #($url, $siteRootUrl)
{
	...
	$url =~ s/((\d|\w)+?:\/\/$ENV{'SERVER_NAME'})?\/le\-filtre\/(.*?)\/(.*)$/$refererUrl = makeUrlAbsolute(urlDecode($4), $siteRootUrl);/segi;
