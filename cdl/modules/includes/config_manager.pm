...
sub createSiteConfig #($siteId)
{
	...
	open(SITE_INI, " > ".$cdlRootPath.$cdlSitesConfigPath.$siteId."/".$siteId.".ini");
...
sub loadConfig #($configFile)
{
	...
	open(CONFIG_FILE, $cdlRootPath.$configFile) || die "Erreur d'ouverture du fichier : $configFile";
...
sub saveConfig #($configFile, $configString)
{
	...
	# Ouverture du fichier de configuration et récupération du flux où lire
	open(CONFIG_FILE_WRITER, " > ".$cdlRootPath.$configFile) || die "Erreur d'ouverture du fichier : $configFile";
