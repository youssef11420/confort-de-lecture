...
sub parseComboItemsToLinks #($selectInnerHtmlCode, $pagePath, $comboItemToLinkTemplateString, $siteId)
{
	...
	my ($selectInnerHtmlCode, $pagePath, $comboItemToLinkTemplateString, $siteId) = @_;
	...
	$selectInnerHtmlCode =~ s/<(option)(\s[^<]*?)?\s(value)\s*=\s*(\"|\')(.*?)\4(.*?)>(.*?)<\/$1>/
		$allItems .= setValueInTemplateString(setValueInTemplateString($comboItemToLinkTemplateString, 'ITEM_URL', getUriFromUrl($5, $pagePath, $siteId)), 'ITEM_NAME', $7);/segi;
...
#	$siteId - identifiant du site parsé
sub parseAllComboToLinks #($htmlCode, $pagePath, $comboToLinkTemplateString, $siteId)
{
	# Extraction des arguments dans une variable locale :
	# - code HTML à traiter
	# - chemin vers la page en cours de traitement
	...
	# - identifiant du site parsé
	my ($htmlCode, $pagePath, $comboToLinkTemplateString, $siteId) = @_;
	...
	$htmlCode =~ s/<!--cdlComboToLink(\s[^<]*?)?-->(.*?)(<select(\s[^<]*?)?>)(.*?)(<\/select>)(.*?)<!--\/cdlComboToLink-->/
		$2.setValueInTemplateString($comboToLinkTemplateString, 'LIST_ITEM', parseComboItemsToLinks($5, $pagePath, getPartOfTemplateString($comboToLinkTemplateString, 'LIST_ITEM'), $siteId)).$7/seg;
