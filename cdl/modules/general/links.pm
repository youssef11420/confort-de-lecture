...
sub parseLinkHrefAttribute #($url, $pagePath, $siteId)
{
	...
	if ($url =~ m/(.*?)(\#.*)/si) {
		$url =~ s/(.*?)(\#.*)/return getUriFromUrl($1, $pagePath, $siteId).$2;/segi;
	}

	# Sinon traiter l'URL en entier
	return getUriFromUrl($url, $pagePath, $siteId);
}
...
#	$pagePath - chemin vers la page en cours de traitement
#	$siteId - identifiant du site parsé
sub parseLinkHref #($htmlCode, $pagePath, $siteId)
{
	# Extraction des arguments dans une variable locale :
	...
	$htmlCode =~ s/(<a(\s[^<]*?)?\s(href))\s*=\s*(\"|\')(.*?)\4/$1."=\"".parseLinkHrefAttribute($5, $pagePath, $siteId)."\""/segi;
