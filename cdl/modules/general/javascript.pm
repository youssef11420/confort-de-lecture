...
sub parseEventListenersAttributes  #($tagAttributes)
{
	...
	$tagAttributes =~ s/\s($eventListenersRegExpString)\s*=\s*(\"|\')(.*?)\2/" ".$1."=".$2.parseJavascriptCode($3, $siteId).$2/segi;
...
sub parseScripts #($htmlCode, $siteRootUrl, $pagePath, $siteId, $parseJavascript)
{
	...
	if ($parseJavascript) {
		...
		$htmlCode =~ s/((<(script)(\s[^<]*?)?>)(.*?)(<\/\3>))/$allScripts .= "\t\t".$2.parseJavascriptCode($5, $siteId).$6."\n"; $1/segi;
		# Transformation du javascript dans les attributs 
		$htmlCode =~ s/(<(\w|\d)+)(\s.*?)>/$1.parseEventListenersAttributes($3, $siteId).">"/segi;
	} else {
...
sub parseJavascriptCodeLine #($jsCode, $siteId)
{
	...
	$jsCode =~ s/(([^;]*?)(\.location)(\.href)?)\s*=\s*(\"|\')(.*?)\5/$1."=".$5."\/le\-filtre\/".$siteId."\/".$6.$5/segi;

	# Transformer l'URL dans l'instruction d'ouverture dans une nouvelle fenêtre pour passer par le script principal
	$jsCode =~ s/(window\.open\()(\"|\')(.*?)\2((\s*,.*?)?\))/$1.$2."\/le\-filtre\/".$siteId."\/".$3.$2.$4/segi;
...
sub parseJavascriptCode #($jsCode, $siteId)
{
	...
	# Parser chaque ligne
	$jsCode =~ s/(.*?)(\n|$)/parseJavascriptCodeLine($1, $siteId).$2/segi;
