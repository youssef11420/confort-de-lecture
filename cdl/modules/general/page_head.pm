...
sub parseMetaAttributes #($tagAttributes, $pagePath, $siteId)
{
	# Extraction des arguments dans une variable locale :
	...
	$tagAttributes =~ s/(\s(content))\s*=\s*(\"|\')(.*?)\sURL\s*=\s*(.*?)\s*\3/
		$1."=".$3.$4." URL=\/le\-filtre\/".$siteId."\/".getUriFromUrl($5, $pagePath, $siteId).$3/segi;
...
#	$pagePath - chemin vers la page en cours de traitement
#	$siteId - identifiant du site parsé
sub parseMetas #($htmlCode, $pagePath, $siteId)
{
	# Extraction des arguments dans une variable locale :
	# - code HTML à traiter
	# - chemin vers la page en cours de traitement
	...
	$htmlCode =~ s/((<meta)(\s[^<]*?)?(\s(http-equiv)\s*=\s*(\"|\')?Refresh\5)(\s[^<]*?)?(\s\/>))/
		$2.parseMetaAttributes($3, $pagePath, $siteId).$4.parseMetaAttributes($7, $pagePath, $siteId).$8/segi;
