...
sub parseMapAreaAttributes #($tagAttributes, $pagePath, $siteId)
{
	# Extraction des arguments dans une variable locale :
	# - code HTML des attibuts à traiter
	# - chemin vers la page en cours de traitement
	# - identifiant du site parsé
	my ($tagAttributes, $pagePath, $siteId) = @_;

	# Transformer l'URL pour passer par le script CDL principal
	$tagAttributes =~ s/(\s(href))\s*=\s*(\"|\')(.*?)\3/$1."=".$3.getUriFromUrl($4, $pagePath, $siteId).$3/segi;
...
#	$siteId - identifiant du site parsé
sub parseMapAreas #($htmlCode, $pagePath, $displayImages, $siteId)
{
	# Extraction des arguments dans une variable locale :
	# - code HTML à traiter
	# - chemin vers la page en cours de traitement
	# - booléen indiquant si l'affichage des images est activé
	# - identifiant du site parsé
	my ($htmlCode, $pagePath, $displayImages, $siteId) = @_;
	...
	if ($displayImages) {
		...
		$htmlCode =~ s/<area(\s[^<]*?)?\s(href)\s*=\s*(\"|\')(.*?)\3(\s[^<]*?)?\s(alt)\s*=\s*(\"|\')(.*?)\7.*?\s\/>/
			"<li><a href=".$3.getUriFromUrl($4, $pagePath, $siteId).$3.">".$8."<\/a><\/li>"/segi;
		$htmlCode =~ s/<area(\s[^<]*?)?\s(alt)\s*=\s*(\"|\')(.*?)\3(\s[^<]*?)?\s(href)\s*=\s*(\"|\')(.*?)\7.*?\s\/>/
			"<li><a href=".$7.getUriFromUrl($8, $pagePath, $siteId).$7.">".$4."<\/a><\/li>"/segi;
	}
